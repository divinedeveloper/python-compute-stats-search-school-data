...
For this project, we'll be using a publically available data source of schools in a CSV format. We'll download the data file and write two Python programs to manipulate the data.


## Part 0: Get the data
* Go to [this page](https://nces.ed.gov/ccd/CCDLocaleCode.asp) and download the dataset described as:
    * Year 2005-2006 (v.1b), States A-I, ZIP (769 KB) CSV File. This is the only file we will be using. We can ignore the other files on this page.
    * We may also find it helpful to consult [the documentation](https://nces.ed.gov/ccd/pdf/sl051bgen.pdf) for this dataset.
    * Unzip the file, rename it to school_data.csv, and put it in the directory where we'll write our Python program.
    * Now we have the data we need to get started.

## Part 1: Load data from CSV and compute stats.
Write a method (or a method per question if we'd prefer) that loads the data out of the CSV file
we downloaded and computes answers to the following questions:
...
* Please implement all of these features using pure python.
    * We may use the following libraries: csv, time, itertools
...
### Guidelines
* When a query doesn't match exactly, we'll need to come up with a set of rules to rank results. In particular, make sure more precise matches show up at the top of the list, and if there isn't an exact match, but there is a close match, some results are returned. There is no perfect set of rules, but we should come up with a set that improve the end user search experience as much as possible.
* Searches should run in real-time, meaning that they should return results to the user in less than 200ms. It's ok to perform data loading and processing up front that takes longer than this if we'd like.
...
### Evaluation
* Accuracy
    * We'll evaluate the accuracy of our search using sample queries. We have included a few below in Test Cases that we can test with (we'll also test with others).
    ...
    * If we see [Next Best Hit], that means that we should include a reasonable hit for the search query, but that there isn't a specific hit that we'll be looking for.
* Performance
...
* Code Quality
    ...
    * Prioritize readability over cleverness. We care less that we can code a solution to this problem in one line, and more that we are able to easily follow your code.
